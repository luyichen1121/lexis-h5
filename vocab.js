...
// SAME WORD FAMILY IS NOT A CONTRAST.
//
// `nurture` vs `nurturing` is not a choice anyone is stuck on — it is one word
// wearing two endings, and the app has a Word family section for exactly that.
// Offering it as a near-synonym wastes one of the few slots that should hold a
// word you could actually pick wrongly (owner: 「这个区别很明显,是词族关系啊」).
//
// `lexisSafeHead` deliberately folds only true INFLECTIONS (v1.109.0), so it
// cannot answer this; derivational endings have to come off for the comparison
// and only for the comparison.
// Endings stripped only for this comparison, longest first. `ment` is NOT in
// the list: torment / moment / cement are not derived words, and stripping it
// would file `torment` and `tortoise`-ish stems together — a false SAME costs a
// real contrast, which is worse than leaving one family member in the list.
var LEXIS_DERIV = ["ations", "ation", "ising", "izing", "ings", "ing", "ers", "er", "ors", "or",
                   "ness", "ities", "ity", "ives", "ive", "ables", "able", "ibles", "ible",
                   "ances", "ance", "ences", "ence", "fully", "ful", "ly", "ies", "ied", "ed", "es", "s"];
function lexisFamStem(w) {
  var x = String(w || "").toLowerCase().trim();
  return x.replace(/e$/, "");
}
function lexisSameFamily(a, b) {
  var x = String(a || "").toLowerCase().trim(), y = String(b || "").toLowerCase().trim();
  if (!x || !y) return false;
  if (x === y) return true;
  var short = x.length <= y.length ? x : y, long = x.length <= y.length ? y : x;
  var seen = { }, queue = [long], out = [];
  // strip up to two endings, comparing after each
  for (var round = 0; round < 2 && queue.length; round++) {
    var next = [];
    for (var qi = 0; qi < queue.length; qi++) {
      var w = queue[qi];
      for (var i = 0; i < LEXIS_DERIV.length; i++) {
        var suf = LEXIS_DERIV[i];
        if (w.length - suf.length < 3 || w.slice(-suf.length) !== suf) continue;
        var stem = w.slice(0, w.length - suf.length);
        if (seen[stem]) continue;
        seen[stem] = 1; next.push(stem); out.push(stem);
      }
    }
    queue = next;
  }
  ...
  for (var oi = 0; oi < out.length; oi++) {
    if (lexisFamStem(out[oi]) === lexisFamStem(short)) return true;
  }
  return false;
}
